...
const validator = (course) => {
  ...
  if(!validation.mininteger(course.DepartmentId, 1, "Course Name")) return validation.errorMsg;
  if (course.DepartmentId < 1) {
    return "Invalid Department Id";
  }
  if(!validation.mininteger(course.UniqueId, 4, "Course Name")) return validation.errorMsg;
  if (course.UniqueId == "" || course.UniqueId.length < 4) {
    return "Invalid Unique Id";
  }
  if(!validation.minlength(course.Code, 2, "Course Name")) return validation.errorMsg;
  if (course.Code == "" || course.Code.length < 2) {
    return "Invalid Course Code";
  }
    if(!validation.mininteger(course.Units, 1, "Course Name")) return validation.errorMsg;
  if (course.Units < 1) {
    return "Invalid Course Units";
  }
    if(!validation.mininteger(course.CourseLevel, 1, "Course Name")) return validation.errorMsg;
  if (course.CourseLevel < 1) {
    return "Invalid Course Level";
  }
    if(!validation.mininteger(course.CourseSemester, 1, "Course Name")) return validation.errorMsg;
  if (course.CourseSemester == 0) {
    return "Invalid Course Semester";
  }
    if(!validation.mininteger(course.Status, 1, "Course Name")) return validation.errorMsg;
  if (course.Status == 0) {
